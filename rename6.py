...
if __name__ == "__main__":
    ...
    # 1. UI File/Folder Selection
    root = tk.Tk()
    root.withdraw() # Hide the main tk window
    root.attributes('-topmost', True) # Force popup to the front on Mac
    
    print("\nHow are your certificates stored?")
    print("[1] In a ZIP file")
    print("[2] In a Folder")
    choice = input("Enter 1 or 2: ").strip()
    
    input_path = ""
    if choice == '1':
        print("\n⏳ Opening file browser... Please select your ZIP file.")
        input_path = filedialog.askopenfilename(
            title="Select Certificates ZIP",
            filetypes=[("ZIP files", "*.zip")]
        )
    elif choice == '2':
        print("\n⏳ Opening file browser... Please select your Folder.")
    ...
    # 2. Output ZIP Naming
    output_zip_name = input("\nEnter the desired name for your final ZIP file (e.g., 'may_batch_results'): ").strip()
    
    # Automatically add .zip extension if the user forgets it
    ...
    try:
        ...
        # Package everything up using the dynamic name
        create_output_zip(success_dir, output_zip_name)
        ...
        print(f"🎉 Saved Final ZIP As   : {output_zip_name}")
        print("=" * 45 + "\n")
